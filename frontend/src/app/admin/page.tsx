    ...
    <div className="min-h-screen bg-[#0f1117] text-slate-200 font-sans" translate="no">
      ...
      <div className="fixed top-4 right-4 z-50 flex flex-col gap-2 pointer-events-none">
        {toasts.map(t => (
          <div key={t.id} className={`px-4 py-3 rounded-lg text-xs font-medium shadow-xl border pointer-events-auto
            ${t.type === 'success' ? 'bg-emerald-900/80 border-emerald-500/40 text-emerald-300' : 'bg-red-900/80 border-red-500/40 text-red-300'}`}>
      ...
      <header className="flex items-center justify-between px-5 py-3 bg-[#161b2e] border-b border-white/5">
        ...
        <div className="flex gap-2 items-center">
          <a href="/admin/relatorios" className="flex items-center gap-1.5 text-[10px] text-violet-400 hover:text-violet-300 border border-violet-500/30 hover:border-violet-500/60 bg-violet-500/10 hover:bg-violet-500/20 px-3 py-1.5 rounded-lg transition-all font-medium">
          ...
          {selectedIds.length > 0 && (
            <button onClick={handleBulkDelete} title="Excluir Selecionados"
              className="flex items-center justify-center w-8 h-8 rounded-md bg-red-600/20 text-red-500 border border-red-500/30 hover:bg-red-600/40 transition-all">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
            </button>
          )}
          <button onClick={criarNovaRequisicao} disabled={loading}
            className="flex items-center gap-2 px-4 py-1.5 rounded-md bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 text-xs font-medium hover:bg-emerald-600/30 transition-all disabled:opacity-50">
            + Novo Pedido
          </button>
          <label className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-blue-800/80 text-blue-200 border border-blue-500/50 text-xs cursor-pointer hover:bg-blue-600 transition-colors">
            <input type="file" accept=".jpg,.jpeg,.png,.webp,.pdf" className="hidden" onChange={handleFileUpload} />
            Importar IA
          </label>
          <button onClick={onLogout} title="Sair" className="text-slate-600 hover:text-slate-400 text-xs px-2 py-1.5 border border-white/5 rounded-lg hover:border-white/10 transition-all">Sair</button>
        </div>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 p-5">
        {[
          { label: 'Total de Pedidos', value: metrics.total, color: 'text-blue-400', badge: 'Ativos' },
          { label: 'Pendentes', value: metrics.pendente, color: 'text-amber-400', badge: 'Aguardando' },
          { label: 'Aprovados', value: metrics.aprovado, color: 'text-emerald-400', badge: 'Solicitação' },
                  ...
                  <td className="px-2 py-3">
                    <button onClick={() => toggleExpand(r.id)} title={isExpanded ? 'Fechar' : 'Ver itens'}
                      className={`w-6 h-6 rounded flex items-center justify-center transition-all text-xs ${
                        parsedItens.length > 0 ? 'text-blue-400 hover:bg-blue-500/20' : 'text-slate-700 cursor-default'
                      } ${isExpanded ? 'bg-blue-500/20 rotate-90' : ''}`}>
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateField(r.id, 'status_final', 'finalizado')} disabled={isFin}
                        className={`px-3 py-1.5 rounded text-[10px] font-medium border whitespace-nowrap transition-colors
                          ${isFin ? 'bg-teal-500/10 text-teal-600/50 border-teal-500/10 cursor-not-allowed' : 'bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/40 border-emerald-500/20'}`}>
