...
const STATUS_MAP: Record<string, {label:string;dot:string;text:string}> = {
  ...
  aprovado:     {label:'Aprovado',   dot:'bg-[#ACBCBF]',             text:'text-[#ACBCBF]'},
...
const inputCls = "bg-transparent w-full outline-none text-[#F4FCFB]/90 placeholder-slate-700 text-xs px-3 py-2";
...
export default function App() {
  ...
  if (!isClient) return <div className="min-h-screen bg-[#243C4C] flex items-center justify-center"><div className="w-6 h-6 border-2 border-[#698696]/30 border-t-blue-500 rounded-full animate-spin" /></div>;

  // ── TELA DE LOGIN / REGISTRO ──
  if (!token) {
    const isLogin = authMode === 'login';
    return (
      <div className="min-h-screen bg-[#243C4C] flex flex-col font-sans relative overflow-hidden">
        {/* Glow de fundo */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[500px] bg-[#698696] text-[#243C4C]/10 blur-[120px] rounded-[100%] pointer-events-none" />
        
        <header className="flex items-center justify-between px-6 py-4 relative z-10">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="logo" className="h-9 w-auto object-contain" />
            <span className="text-sm font-semibold text-[#F4FCFB]">Gestão de Suprimentos</span>
          </div>
        ...
        <main className="flex-1 flex items-center justify-center p-4 relative z-10">
          <div className="w-full max-w-sm">
            <div className="text-center mb-8">
              <h1 className="text-xl font-semibold text-[#F4FCFB] mb-1">{isLogin ? 'Bem-vindo de volta' : 'Crie sua conta'}</h1>
              <p className="text-xs text-slate-400">Portal de Requisição de Materiais</p>
            </div>
            
            <form onSubmit={handleAuth} className="bg-[#5289AD]/80 backdrop-blur-md border border-white/5 rounded-2xl p-6 space-y-4 shadow-2xl">
              {!isLogin && (
                <div>
              ...
              <button type="submit" disabled={loading}
                className="w-full py-2.5 rounded-xl bg-[#698696] text-[#243C4C] hover:bg-[#ACBCBF] hover:text-[#243C4C] disabled:opacity-40 text-white text-sm font-medium transition-colors mt-2">
...
function PortalContent({ token, user, onLogout }: { token: string; user: User; onLogout: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-[#243C4C] text-[#F4FCFB]/90 font-sans">
      <header className="flex items-center justify-between px-6 py-4 bg-[#5289AD] border-b border-white/5 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <img src="/logo.png" alt="Gestão de Suprimentos" className="h-10 w-auto object-contain" />
          <div>
            <h1 className="text-sm font-semibold text-[#F4FCFB]">Gestão de Suprimentos</h1>
            <p className="text-[10px] text-slate-500">Portal do Engenheiro</p>
          </div>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-[#698696] text-[#243C4C]/20 text-[#698696] flex items-center justify-center text-xs font-bold border border-[#698696]/30">
            ...
            <div className="hidden sm:block">
              <div className="flex items-center gap-1.5">
                ...
                {user.role === 'admin' && (
                  <span className="bg-[#ACBCBF]/20 text-[#ACBCBF] text-[8px] font-bold px-1.5 py-0.5 rounded uppercase">Admin</span>
                )}
              </div>
      ...
      <main className="max-w-4xl mx-auto px-5 py-10 space-y-12">
        <section>
          <div className="mb-5 flex items-center justify-between">
            <div>
              <h2 className="text-base font-semibold text-[#F4FCFB]">Nova Requisição de Material</h2>
              <p className="text-xs text-slate-500 mt-1">Preencha o formulário e clique em enviar. O número é gerado automaticamente.</p>
            </div>
          </div>

          {success ? (
            <div className="border-2 border-[#ACBCBF]/30 rounded-2xl p-16 bg-[#ACBCBF]/5 text-center">
              <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-[#ACBCBF]/10 border border-[#ACBCBF]/20 flex items-center justify-center text-3xl">✓</div>
              <p className="text-base font-semibold text-[#ACBCBF]">Requisição enviada com sucesso!</p>
            ...
            <form onSubmit={handleSubmit}>
              <div className="rounded-2xl overflow-hidden border border-white/10 text-xs">
                ...
                <div className="grid grid-cols-[1fr_auto] bg-[#5289AD]/80 border-b border-white/10">
                  ...
                  <div className="px-6 py-3 border-l border-white/10 text-right">
                    ...
                    <p className="text-lg font-bold text-[#698696] font-mono leading-tight">{nextNum}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-[1fr_100px_160px] border-b border-white/10">
                  ...
                  <div className={`${cellCls} flex items-center justify-center border-t-0`}>
                    <span className="text-sm font-bold text-[#698696]/80 font-mono">{cc}</span>
                  </div>
                ...
                <div className="grid grid-cols-[1fr_90px_100px_90px_90px] bg-[#5289AD]/80 border-b border-white/10">
                  ...
                  <div key={i} className="grid grid-cols-[1fr_90px_100px_90px_90px] border-b border-white/10 group hover:bg-white/3 transition-colors">
                    ...
                    <div className="border-r border-white/10">
                      <select value={it.unidade} onChange={e=>setItem(i,'unidade',e.target.value)} className="bg-transparent w-full h-full outline-none text-slate-300 text-xs px-3 py-2 cursor-pointer">
                        {UNIDADES.map(u=><option key={u} className="bg-[#5289AD]/80" value={u}>{u}</option>)}
                      </select>
                    </div>
                    <div className="border-r border-white/10">
                ...
                <div className="border-b border-white/10">
                  <button type="button" onClick={addItem} className="w-full px-4 py-2.5 text-[11px] text-[#698696] hover:text-[#698696]/80 hover:bg-[#ACBCBF] hover:text-[#243C4C]/5 transition-colors text-left flex items-center gap-2"><span className="text-base leading-none font-bold">+</span> Adicionar item</button>
                </div>

                <div className="grid grid-cols-3 bg-[#5289AD]/80 border-b border-white/10">
              ...
              <div className="flex gap-3 mt-4">
                <button type="submit" disabled={loading} className="flex-1 py-2.5 rounded-xl bg-[#698696] text-[#243C4C] hover:bg-[#ACBCBF] hover:text-[#243C4C] text-white text-sm font-semibold transition-colors disabled:opacity-50">
        ...
        <section>
          <h2 className="text-base font-semibold text-[#F4FCFB] mb-1">{user.role === 'admin' ? 'Todas as Requisições' : 'Meu Histórico de Pedidos'}</h2>
          ...
          <div className="bg-[#5289AD] border border-white/5 rounded-2xl overflow-hidden">
